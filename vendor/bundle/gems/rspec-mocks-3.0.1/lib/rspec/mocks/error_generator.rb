module RSpec
  module Mocks
    ...
    # Raised when a spec double is used after it has been torn
    # down (typically at the end of an rspec-core example).
    ExpiredTestDoubleError = Class.new(MockExpectationError)

    # Raised when doubles or partial doubles are used outside of the per-spec lifecycle.
    ...
    class ErrorGenerator
      ...
      def raise_only_valid_on_a_partial_double(method)
        __raise "#{intro} is a pure spec double. `#{method}` is only " +
