...
module RSpec
  ...
  # a spec or example. They exist primarily for integration with
  # spec frameworks (such as rspec-core).
  module Mocks
    # Performs per-spec/example setup. This should be called before
    # an spec or example begins.
    def self.setup
      @space_stack << (@space = space.new_scope)
    end

    # Verifies any message expectations that were set during the
    # spec or example. This should be called at the end of an example.
    def self.verify
      space.verify_all
    end

    # Cleans up all spec double state (including any methods that were
