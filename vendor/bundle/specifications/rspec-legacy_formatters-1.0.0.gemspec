# -*- encoding: utf-8 -*-

Gem::Specification.new do |s|
  s.name = "rspec-legacy_formatters"
  s.version = "1.0.0"

  s.required_rubygems_version = Gem::Requirement.new(">= 0") if s.respond_to? :required_rubygems_version=
  s.authors = ["Jon Rowe"]
  s.cert_chain = ["-----BEGIN CERTIFICATE-----\\n-----END CERTIFICATE-----\n"]
  s.date = "2014-06-01"
  s.description = "Support for RSpec 2.x formatters on 3.x"
  s.email = "rspec@googlegroups.com"
  s.homepage = "http://github.com/rspec/rspec-legacy_formatters"
  s.licenses = ["MIT"]
  s.rdoc_options = ["--charset=UTF-8"]
  s.require_paths = ["lib"]
  s.required_ruby_version = Gem::Requirement.new(">= 1.8.7")
  s.rubygems_version = "2.0.3"
  s.summary = "rspec-legacy_formatters-1.0.0"

  if s.respond_to? :specification_version then
    s.specification_version = 4

    if Gem::Version.new(Gem::VERSION) >= Gem::Version.new('1.2.0') then
      s.add_runtime_dependency(%q<rspec-core>, [">= 3.0.0.beta2"])
      s.add_runtime_dependency(%q<rspec-support>, [">= 3.0.0.beta2"])
      s.add_development_dependency(%q<cucumber>, ["~> 1.3"])
      s.add_development_dependency(%q<aruba>, ["~> 0.5"])
      s.add_development_dependency(%q<rake>, ["~> 10.0.0"])
      s.add_development_dependency(%q<fuubar>, ["= 1.3.2"])
      s.add_development_dependency(%q<nyan-cat-formatter>, ["= 0.5.2"])
      s.add_development_dependency(%q<rspec-instafail>, ["= 0.2.4"])
      s.add_development_dependency(%q<rspec-extra-formatters>, ["= 1.0.0"])
      s.add_development_dependency(%q<fivemat>, ["= 1.2.1"])
    else
      s.add_dependency(%q<rspec-core>, [">= 3.0.0.beta2"])
      s.add_dependency(%q<rspec-support>, [">= 3.0.0.beta2"])
      s.add_dependency(%q<cucumber>, ["~> 1.3"])
      s.add_dependency(%q<aruba>, ["~> 0.5"])
      s.add_dependency(%q<rake>, ["~> 10.0.0"])
      s.add_dependency(%q<fuubar>, ["= 1.3.2"])
      s.add_dependency(%q<nyan-cat-formatter>, ["= 0.5.2"])
      s.add_dependency(%q<rspec-instafail>, ["= 0.2.4"])
      s.add_dependency(%q<rspec-extra-formatters>, ["= 1.0.0"])
      s.add_dependency(%q<fivemat>, ["= 1.2.1"])
    end
  else
    s.add_dependency(%q<rspec-core>, [">= 3.0.0.beta2"])
    s.add_dependency(%q<rspec-support>, [">= 3.0.0.beta2"])
    s.add_dependency(%q<cucumber>, ["~> 1.3"])
    s.add_dependency(%q<aruba>, ["~> 0.5"])
    s.add_dependency(%q<rake>, ["~> 10.0.0"])
    s.add_dependency(%q<fuubar>, ["= 1.3.2"])
    s.add_dependency(%q<nyan-cat-formatter>, ["= 0.5.2"])
    s.add_dependency(%q<rspec-instafail>, ["= 0.2.4"])
    s.add_dependency(%q<rspec-extra-formatters>, ["= 1.0.0"])
    s.add_dependency(%q<fivemat>, ["= 1.2.1"])
  end
end
